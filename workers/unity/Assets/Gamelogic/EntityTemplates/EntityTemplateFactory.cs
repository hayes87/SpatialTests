    ...
    public class EntityTemplateFactory : MonoBehaviour
    {
        ...
        public static Entity CreatePlayerTemplate(string clientId, EntityId playerCreatorId)
        {
			var playerTemplate = EntityBuilder.Begin()
				...
				.SetReadAcl(CommonRequirementSets.PhysicsOrVisual)
				.AddComponent(new PlayerTransform.Data(Improbable.Vector3f.ZERO, new Improbable.Core.Quaternion(0f, 0f, 0f, 0f), Improbable.Vector3f.ZERO),CommonRequirementSets.SpecificClientOnly(clientId))
                .AddComponent(new Rotation.Data(Quaternion.identity.ToNativeQuaternion()), CommonRequirementSets.PhysicsOnly)
